...
SELECT --TOP 1000
	...
	-- SELECT COUNT(*)
		FROM {prefix}{pc_dataset}.pc_contact
		LEFT OUTER JOIN (select * from {prefix}{pc_dataset}.pc_accountcontact Where DATE(_PARTITIONTIME) = DATE('{ydate}')) pc_accountcontact ON pc_contact.ID = pc_accountcontact.Contact
		LEFT OUTER JOIN (select * from {prefix}{pc_dataset}.pc_account Where DATE(_PARTITIONTIME) = DATE('{ydate}')) pc_account  ON pc_account.ID = pc_accountcontact.Account
		LEFT OUTER JOIN (select * from {prefix}{pc_dataset}.pc_accountcontactrole Where DATE(_PARTITIONTIME) = DATE('{ydate}'))pc_accountcontactrole ON pc_accountcontact.ID = pc_accountcontactrole.AccountContact
		LEFT OUTER JOIN {prefix}{pc_dataset}.pctl_accountcontactrole ON pc_accountcontactrole.Subtype = pctl_accountcontactrole.ID
		INNER JOIN {prefix}{pc_dataset}.pc_address ON pc_address.ID = pc_contact.PrimaryAddressID
		LEFT OUTER JOIN {prefix}{pc_dataset}.pctl_gendertype ON pctl_gendertype.ID = pc_contact.Gender
		LEFT OUTER JOIN {prefix}{pc_dataset}.pctl_contact ON pc_contact.Subtype = pctl_contact.ID
		...
		AND pctl_accountcontactrole.ID IN (16, 14)		--NamedInsured /*14 = Located With, 2 = AccountHolder */
		AND (pc_contact.PublicID not LIKE 'system%' AND pc_contact.PublicID not like 'default%')	/* how else to get rid of garbage ? */
		AND DATE(pc_contact._PARTITIONTIME) = DATE(DATE('{ydate}'))
    --AND DATE(pc_accountcontact._PARTITIONTIME) = DATE(DATE('{ydate}'))
    --AND DATE(pc_account._PARTITIONTIME) = DATE(DATE('{ydate}'))
    --AND DATE(pc_accountcontactrole._PARTITIONTIME) = DATE(DATE('{ydate}'))
      AND DATE(pc_address._PARTITIONTIME) = DATE(DATE('{ydate}'))
		--AND pc_contact.ID = 2319 --79327
		--AND COALESCE(pc_contact.Retired,0) = 0	/* not sure if we include this; if we join to Policy then no */
    ...
    --AND DATE(pc_accountcontact._PARTITIONTIME) = DATE('{ydate}')
    --AND DATE(pc_account._PARTITIONTIME) = DATE('{ydate}')
    --AND DATE(pc_accountcontactrole._PARTITIONTIME) = DATE('{ydate}')
    AND DATE(pc_address._PARTITIONTIME) = DATE('{ydate}')
		--AND pc_contact.ID = 2319 --79327
		--AND COALESCE(pc_contact.Retired,0) = 0	/* not sure if we include this; if we join to Policy then no */
	...
	UNION DISTINCT
		...
		INNER JOIN (	SELECT pc_contact.ID ContactID
						...
						FROM {prefix}{pc_dataset}.pc_contact
							LEFT OUTER JOIN {prefix}{pc_dataset}.pc_contactaddress
								on pc_contact.ID = pc_contactaddress.ContactID
								AND DATE(pc_contactaddress._PARTITIONTIME) = DATE('{ydate}')
								AND DATE(pc_contact._PARTITIONTIME) = DATE('{ydate}')
						...
						ON pc_contact.ID = FirstMailingAddress.ContactID
					LEFT OUTER JOIN (Select * from {prefix}{pc_dataset}.pc_address Where DATE(_PARTITIONTIME) = DATE('{ydate}')) PC_ADDRESS_MAILING_1
						ON FirstMailingAddress.MailingAddressID = PC_ADDRESS_MAILING_1.ID
    ...
    --AND DATE(pc_account._PARTITIONTIME) = DATE('{ydate}')
    --AND DATE(pc_accountcontactrole._PARTITIONTIME) = DATE('{ydate}')
    --AND DATE(PC_ADDRESS_MAILING_1 ._PARTITIONTIME) = DATE('{ydate}')
		--AND pc_contact.ID = 2319 --79327 and FirstMailingAddress.MailingAddressID = 4949295
		--AND COALESCE(pc_contact.Retired,0) = 0	/* not sure if we include this; if we join to Policy then no */
	...
	UNION DISTINCT
		...
		INNER JOIN (	SELECT pc_contact.ID ContactID
							...
							on pc_contact.ID = ThirdMailingAddress.ContactID
						LEFT OUTER JOIN  (select * from {prefix}{pc_dataset}.pc_address Where DATE(_PARTITIONTIME) = DATE('{ydate}')) PC_ADDRESS_MAILING_3
							ON ThirdMailingAddress.MailingAddressID = PC_ADDRESS_MAILING_3.ID
    ...
    --AND DATE(pc_account._PARTITIONTIME) = DATE('{ydate}')
    --AND DATE(pc_accountcontactrole._PARTITIONTIME) = DATE('{ydate}')
    --AND DATE(PC_ADDRESS_MAILING_3 ._PARTITIONTIME) = DATE('{ydate}')
		--AND pc_contact.ID = 2319 --79327
		--AND COALESCE(pc_contact.Retired,0) = 0	/* not sure if we include this; if we join to Policy then no */
)
	) CustomerData
