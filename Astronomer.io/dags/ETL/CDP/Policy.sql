...
v7	- Changed join for IsActivePolicy to use Typecode from pctl_policyperiodstatus; 
	- Corrected PA field from IsActive to IsActivePolicy
v8	- Added JOIN to pctl_grouptype to limit records to 'agency_jmic' for AgencyName field
v9	- Corrected TransactionRank to pull all valid policy-term records and rank to get latest
	- Possibly new name for PolicyPeriodCreateTime -- PeriodTransactionDate
	- Changed to subquery to get payment plan summary
	- Only use PolicyPeriod records with valid ModelDate (per Hussein = a valid record in GW)
...
FROM (
		...
		-- Get Max PeriodEnd Date
			...
			LEFT JOIN	(
				...
				AND DATE(pc_policyperiod._PARTITIONTIME) = DATE('{date}')
				AND pc_policy.ProductCode IN ('JMIC_PL', 'JPAPersonalArticles')
				AND pc_paymentplansummary.ID IS NOT NULL
			) MaxPaymentPlan
