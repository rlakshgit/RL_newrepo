...
Types of Transactions Possible - from dimPolicy

Jobtype				N
Renewal				3690962
Policy Change		1764025
Cancellation		1295220
Submission			1102859
Reinstatement		244550
Rewrite				143736
Issuance			1228


v4 - [X] add policy number
	 [X] brand details
	 [X] convert to trxn_summary
	 [X] Add itemclass - pending above
	 [X] Wearer contact id

v6 - [X] Troubleshoot isItemInactive - is it that we only write premium on active items? Do we need future state??
	 [X] Use Transaction Posted Date to filter

v7 - [X] Add ItemValue
	 [-] Union all this query to get Policy Changes with no premium - Actually don't think needed based on conversations with Acquia

v8 - [X] Guard against NULL Wearers, don't allow "pc:"

20201117 needs:
	[X] Need isItemInactive flag that works
	[ ] Located with has to go through policycontactrole as Janice showed in her email
	[X] Item Value adding duplicates
	[ ] Remove dependency on DW_STAGE (was needed to get sample data out - IsItemInactive flag)

v11 -	[X] Located with has to go through policycontactrole as Janice showed in her email
		[X] Remove dependency on DW_STAGE (was needed to get sample data out - IsItemInactive flag)
		[X] Added field for primary key: pctl_policyline.TYPECODE AS PolicyLineCode

11/24/2020 Questions
[ ]	This procedure combines all PJ Direct & Ceded with PA Direct and Ceded:  Should these be split into multiple procs and feed 1 table?
[X]	This procedure is at the GW [financial] Transaction level; also, added unique ID field: TransactionPublicID to match back to Transactions table/feed
[X] Added field: TransactionAmount

v12 -	[X] Fixed issue with IsItemActive flag
		[X] Remove use of pc_policyperiod as perCost -- was causing issues tying PolicyPeriodPublicID to the JobNumber at the transaction level

v13	-	[X] Replaced hardcoded currency value with table join at policy line
		[X] Added CTE to join in PLecom ExternalApplicaitonID per AccountNUmber

OPEN ISSUES: Test Policy 24-1039286
[]	-	item #3 not showing in CDP or PE output  ( I could not find the cost id’s in the transaction table -- pcx_jmtransaction)
[]	-	items #5 & 6 are showing in output but are not showing the IsItemActive flag =1 (or InActive)

v14	-	[X] Restrict output to BOUND transaction status only (in Transaction header file feed)
...
/***** TESTING ONLY ********
		DECLARE @HIGHAccNbr varchar(15), @LOWAccNber varchar(15)
		SELECT @HIGHAccNbr = '3001200057' , @LOWAccNber= '3001200057'
/***************************
...
AS	(
		...
		FROM (
			  ...
			  FROM {prefix}{plecom_dataset}.t_Application
			  WHERE DATE(t_Application._PARTITIONTIME) = DATE('{ydate}')
			) PlecomAccounts
		WHERE 1=1
		AND PlecomAccounts.ApplicationRank = 1
	)	
...
FROM (
	(
		...
		SELECT * from (
                    ...
                    SELECT
                        ...
                        ,pctl_reasoncode.NAME AS PolicyCancelReason
                        ,pctl_policychangerea_jmic_pl.NAME AS PolicyChangeReason
                        ,cte_ExtAppIdy.ExternalApplicationKey
                        ,pc_policyperiod.PolicyNumber
                        ,pc_policyperiod.PolicyTermID AS PolicyTermID
                        ,pc_account.AccountNumberDenorm	AS AccountNumber		--feels important to include, don't see
                        ,pctl_Job.NAME AS TransactionType
                    ...
                    -- Jewelry Item Coverage
                        ...
                        LEFT JOIN {prefix}{pc_dataset}.pctl_classcodetype_jmic_pl AS pctl_classcodetype_jmic_pl ON pctl_classcodetype_jmic_pl.ID = pcx_jewelryitem_jmic_pl.ClassCodeType
                    --logic to get the "Located WITH contact"
                        LEFT OUTER JOIN (select * from {prefix}{pc_dataset}.pc_policycontactrole Where Date(_PARTITIONTIME)=DATE('{ydate}')) AS LocatedWithRole
                            ON LocatedWithRole.FixedID = pcx_jewelryitem_jmic_pl.LocatedWith
                            AND LocatedWithRole.BranchID = pcx_jewelryitem_jmic_pl.BranchID
                            AND COALESCE(pcx_cost_jmic.EffectiveDate,perCost.EditEffectiveDate) >= COALESCE(LocatedWithRole.EffectiveDate,perCost.PeriodStart)
                            AND COALESCE(pcx_cost_jmic.EffectiveDate,perCost.EditEffectiveDate) < COALESCE(LocatedWithRole.ExpirationDate,perCost.PeriodEnd)
                        LEFT OUTER JOIN (select * from {prefix}{pc_dataset}.pc_accountcontactrole Where Date(_PARTITIONTIME)=DATE('{ydate}'))pc_accountcontactrole
                        ON pc_accountcontactrole.ID = LocatedWithRole.AccountContactRole
                        LEFT OUTER JOIN (select * from {prefix}{pc_dataset}.pc_accountcontact Where Date(_PARTITIONTIME)=DATE('{ydate}'))pc_accountcontact
                        ON pc_accountcontact.ID = pc_accountcontactrole.AccountContact
                        LEFT OUTER JOIN (select * from {prefix}{pc_dataset}.pc_contact Where Date(_PARTITIONTIME)=DATE('{ydate}')) AS LocatedWithContact
                        ON LocatedWithContact.ID = pc_accountcontact.Contact
                        LEFT JOIN cte_ExtAppIdy ON cte_ExtAppIdy.AccountNumber = pc_account.AccountNumberDenorm --COLLATE Latin1_General_CI_AS

                    WHERE	1 = 1
                    --AND pcx_jmtransaction.PostedDate IS NOT NULL
                    AND pctl_policyperiodstatus.NAME = 'Bound'
                    AND DATE(perCost._PARTITIONTIME) = DATE('{ydate}')
                        AND DATE(pcx_jmtransaction._PARTITIONTIME) = DATE('{ydate}')
                    ...
                    AND DATE(pc_policycontactrole._PARTITIONTIME) = DATE('{ydate}')
                    AND DATE(pc_contact._PARTITIONTIME) = DATE('{ydate}')
                    AND DATE(pcx_jwryitemcov_jmic_pl._PARTITIONTIME) = DATE('{ydate}')
                    AND DATE(pcx_jewelryitem_jmic_pl._PARTITIONTIME) = DATE('{ydate}')
	...
	(
		...
		SELECT * FROM (
                ...
                AND DATE(pc_policycontactrole._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_contact._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pcx_jmpersonallinecov._PARTITIONTIME) = DATE('{ydate}')
	    )
	...
	(
		...
		(
			...
			SELECT * FROM (
                ...
                WHERE	1 = 1
                AND pctl_policyperiodstatus.NAME = 'Bound'
                AND DATE(pcx_plcededpremiumtrans_jmic._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pcx_plcededpremiumjmic._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pcx_cost_jmic._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_policyperiod._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_job._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_policy._PARTITIONTIME) = DATE('{ydate}')
		...
		(
			...
			SELECT * FROM (
                ...
                SELECT
                    ...
                    INNER JOIN (select * from {prefix}{pc_dataset}.pcx_personalarticlecov_jm Where Date(_PARTITIONTIME)=DATE('{ydate}'))pcx_personalarticlecov_jm
                        ON pcx_personalarticlecov_jm.FixedID = pcx_jpacost_jm.PersonalArticleCov
                        AND pcx_personalarticlecov_jm.BranchID = pcx_jpacost_jm.BranchID
                        AND COALESCE(pcx_jpacost_jm.EffectiveDate,perCost.EditEffectiveDate) >= COALESCE(pcx_personalarticlecov_jm.EffectiveDate,perCost.PeriodStart)
                        AND COALESCE(pcx_jpacost_jm.EffectiveDate,perCost.EditEffectiveDate) < COALESCE(pcx_personalarticlecov_jm.ExpirationDate,perCost.PeriodEnd)
                    LEFT OUTER JOIN (select * from {prefix}{pc_dataset}.pcx_personalarticle_jm Where Date(_PARTITIONTIME)=DATE('{ydate}'))pcx_personalarticle_jm
                        ON pcx_personalarticle_jm.ID = pcx_jpacost_jm.PersonalArticle
                        AND pcx_personalarticle_jm.BranchID = pcx_jpacost_jm.BranchID
                        AND COALESCE(pcx_jpacost_jm.EffectiveDate,perCost.EditEffectiveDate) >= COALESCE(pcx_personalarticle_jm.EffectiveDate,perCost.PeriodStart)
                        AND COALESCE(pcx_jpacost_jm.EffectiveDate,perCost.EditEffectiveDate) < COALESCE(pcx_personalarticle_jm.ExpirationDate,perCost.PeriodEnd)
                ...
                AND pcx_jpatransaction_jm.PublicID IS NOT NULL
                AND pctl_policyperiodstatus.NAME = 'Bound'
                AND DATE(pcx_jpatransaction_jm._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pcx_jpacost_jm._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_policyperiod._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(percost._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_job._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_policy._PARTITIONTIME) = DATE('{ydate}')
	...
	(
		...
		(
            ...
            --	AND DATE(pc_policyterm._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_policycontactrole._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_contact._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pcx_personalartcllinecov_jm._PARTITIONTIME) = DATE('{ydate}')
			...
			SELECT * FROM (
                ...
                -- select pcx_jpatransaction_jm.*
                    ...
                    INNER JOIN (Select * from {prefix}{pc_dataset}.pcx_personalarticlecov_jm Where Date(_PARTITIONTIME)=DATE('{ydate}'))pcx_personalarticlecov_jm
                        ...
                        AND COALESCE(pcx_jpacost_jm.EffectiveDate,pc_policyperiod.EditEffectiveDate) >= COALESCE(pcx_personalarticlecov_jm.EffectiveDate,pc_policyperiod.PeriodStart)
                        AND COALESCE(pcx_jpacost_jm.EffectiveDate,pc_policyperiod.EditEffectiveDate) < COALESCE(pcx_personalarticlecov_jm.ExpirationDate,pc_policyperiod.PeriodEnd)
                    INNER JOIN (Select * from {prefix}{pc_dataset}.pcx_personalarticle_jm Where Date(_PARTITIONTIME)=DATE('{ydate}'))pcx_personalarticle_jm
                        ON pcx_personalarticle_jm.ID = pcx_jpacost_jm.PersonalArticle
                        AND pcx_personalarticle_jm.BranchID = pcx_jpacost_jm.BranchID
                        AND COALESCE(pcx_jpacost_jm.EffectiveDate,pc_policyperiod.EditEffectiveDate) >= COALESCE(pcx_personalarticle_jm.EffectiveDate,pc_policyperiod.PeriodStart)
                        AND COALESCE(pcx_jpacost_jm.EffectiveDate,pc_policyperiod.EditEffectiveDate) < COALESCE(pcx_personalarticle_jm.ExpirationDate,pc_policyperiod.PeriodEnd)
                    ...
                    LEFT OUTER JOIN {prefix}{pc_dataset}.pctl_jpaitemgendertype_jm ON pcx_personalarticle_jm.ItemGenderType= pctl_jpaitemgendertype_jm.ID
                    --logic to get the "Located WITH contact"
                    LEFT OUTER JOIN (Select * from {prefix}{pc_dataset}.pc_policycontactrole Where Date(_PARTITIONTIME)=DATE('{ydate}'))
                    AS LocatedWithRole
                        ON LocatedWithRole.FixedID = pcx_personalarticle_jm.LocatedWith
                        AND LocatedWithRole.BranchID = pcx_personalarticle_jm.BranchID
                        AND COALESCE(pcx_jpacost_jm.EffectiveDate,pc_policyperiod.EditEffectiveDate) >= COALESCE(LocatedWithRole.EffectiveDate,pc_policyperiod.PeriodStart)
                ...
                --AND pc_job.JobNumber = '7000010'
                AND pctl_policyperiodstatus.NAME = 'Bound'
                AND DATE(pcx_jpacededpremiumtrans_jm._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pcx_jpacededpremium_jm._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pcx_jpacost_jm._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_policyperiod._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_job._PARTITIONTIME) = DATE('{ydate}')
                AND DATE(pc_policy._PARTITIONTIME) = DATE('{ydate}')
