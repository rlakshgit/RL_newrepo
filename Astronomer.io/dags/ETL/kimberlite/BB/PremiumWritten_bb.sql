/**********************************************************************************************************************************
 ***** Kimberlite ******
		PremiumWritten-BuildingBlock.sql
**********************************************************************************************************************************/
/*
----------------------------------------------------------------------------------------------------------------------------------
 *****  Change History  *****

    03/04/2022  DROBAK      Initial create
...
----------------------------------------------------------------------------------------------------------------------------------
    ...
    PA Direct
    PJ Direct
        Note: Ceded not inlcuded at this time though untested commented sections exist below
-----------------------------------------------------------------------------------------------------------------------------------
*/	
/**********************************************************************************************************************************/
/*
    INSERT `{project}.{dest_dataset}.PremiumWritten`
    (
        SourceSystem,				--new
		--CompanyKey,			--need to add but cannot until we get business unit table built in kimberlite
		FinancialTransactionKey,	--new
        ...
        JobNumber,
        PolicyNumber,
		PolicyExpirationDate,		--new
		PolicyTransEffDate,			--new
        ItemNumber,
		ModelNumber,				--new aka PolicyVersion -- what name do we use in Gold layer?
        CoverageCode,
        TransactionType,
        TransactionCount,
	...
	--Possible other fields to add to Kimberlite Building Block PremiumWritten
	--FinancialTransaction.TransactionPublicID or FinancialTransactionKey
*/
    -- CREATE OR REPLACE TABLE `{project}.{dest_dataset}.PremiumWritten`
    -- AS
    DELETE `{project}.{dest_dataset}.PremiumWritten_bb` WHERE bq_load_date = DATE({partition_date});
    INSERT INTO `{project}.{dest_dataset}.PremiumWritten_bb`
    (
        	SourceSystem
    ...
    LEFT JOIN (SELECT RiskLocationKey, PolicyTransactionKey FROM `{project}.{core_dataset}.RiskLocationBusinessOwners` WHERE bq_load_date=DATE({partition_date})) RiskLocationBusinessOwners
...
UNION ALL 
    ...
    LEFT JOIN (SELECT RiskLocationKey, PolicyTransactionKey FROM `{project}.{core_dataset}.RiskLocationIM` WHERE bq_load_date=DATE({partition_date})) RiskLocationIM
