-- tag: ClaimFinancialTransactionLinePJDirect_MissingByX - tag ends/
/*** ClaimFinancialTransactionLinePJDirect_MissingByX.sql ***

	*****  Change History  *****

	09/28/2022	DROBAK		Updated select output; added PostedDate Clause
	11/07/2022	DROBAK		Kimberlite Table Name Change
	05/09/2023	DROBAK		Update 2 hardcoded paths to parameters
-----------------------------------------------------------------------------
*/
--Missing Trxn
...
(
  ...
  SELECT 'LineCode','JMICPJLine' UNION ALL
  SELECT 'UWCompany', 'uwc:10'
)
	...
	WHERE cc_transaction.PublicID NOT IN (SELECT TransactionPublicID FROM `{project}.{dest_dataset}.ClaimFinancialTransactionLinePJDirect` WHERE bq_load_date = DATE({partition_date}))
	...
	--LEFT JOIN `{project}.{cc_dataset}.cctl_transactionstatus` AS cctl_transactionstatus 
	--	ON cctl_transactionstatus.ID = cc_transaction.Status
	WHERE cc_transaction.PublicID not in (select TransactionPublicID from `qa-edl.B_QA_ref_kimberlite.dar_ClaimFinancialTransactionLinePJDirect`)
	--AND COALESCE(cctl_underwritingcompanytype.TYPECODE, '?') NOT IN ('FedNat', 'TWICO') --excludes FedNat & TWICO claims from being processed
	--AND COALESCE(cc_transaction.AuthorizationCode_JMIC,'') NOT IN ('Credit Card Payment Pending Notification')
	--AND cctl_transactionstatus.TYPECODE IN ('submitting','pendingrecode','pendingstop','pendingtransfer','pendingvoid','submitted','recoded','stopped','transferred','voided')
*/