...
sql_source_db = 'BillingCenter'
sql_source_db_abbr = 'bc'
ct_enable = True
...
with DAG(
    ...
    try:
        KIMBERLITE_BC_TABLES = Variable.get('prioritized_bc_tables')
    except:
        Variable.set('prioritized_bc_tables','dbo.bc_policyperiod,dbo.bctl_policyperiodbillingmethod,dbo.bc_account,dbo.bc_acctpmntinst,dbo.bc_paymentinstrument,dbo.bctl_paymentmethod,dbo.bc_invoice,dbo.bctl_invoicestatus,dbo.bc_accountcontact,dbo.bctl_accounttype,dbo.bc_contact')
        KIMBERLITE_BC_TABLES = Variable.get('prioritized_bc_tables')


    KIMBERLITE_BC_TABLES = KIMBERLITE_BC_TABLES.split(',')
    bit_set = CompletionOperator(task_id='set_source_bit',
                                 source='BillingCenter_Prioritized_Tables',
                                 mode='SET')
    ...
    for table in KIMBERLITE_BC_TABLES:  # [:400]:
        ...
        audit_sql = '''USE {source};
                                                            ...
                                                            order by TableName;'''.format(source=sql_source_db)
