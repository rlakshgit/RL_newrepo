...
non_partitioned_tables = ['gw_gl_LobMap','gw_gl_ChargeMap']
##
#source_dag

with DAG(
        'ProductExtension_PC_Schema_dag',
        schedule_interval = '0 14 * * *',#"@daily",#dt.timedelta(days=1), #'0 23 1 * *',
        catchup=True,
        max_active_runs=1,
        default_args=default_dag_args) as dag:

    source_target_table_list = '{sql_source_db_abbr}_pc_schema_table_list'.format(
    ...
    table_list = MsSqlToGoogleCloudStorageOperator(
                                                    ...
                                                    sql='''SELECT  CONCAT(TABLE_SCHEMA,'.',TABLE_NAME) FROM [{sql_source_db}].information_schema.tables where TABLE_SCHEMA = 'pc' '''.format(sql_source_db=sql_source_db),
                                                    bucket=base_bucket,
    ...
    bit_set = CompletionOperator(task_id='set_source_bit',
                                 source='ProductExtension_PC_Schema',
                                 mode='SET')
