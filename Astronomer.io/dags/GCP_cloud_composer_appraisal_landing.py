...
from plugins.operators.jm_mongodb_to_gcs_appraisal import Mongodbtogcs
...
source='appraisal'
source_abbr = 'appraisal'
collection='appraisaldocuments'
database_name='platform-appraisal'
...
confFilePath = os.path.join(configuration.get('core', 'dags_folder'), r'zing_appraisal', r'ini', r"appraisal_config.ini")
...
with DAG(
        'zing_appraisal_landing_dag',
        schedule_interval='0 10 * * *',  # "@daily",#dt.timedelta(days=1), #'0 23 1 * *',
        catchup=True,
        max_active_runs=1,
        default_args=default_args) as dag:
    refine_dataset = folder_prefix + 'ref_zing_' + source_abbr
    entity_list = ['appraisal', 'appraisalintrialsubscription', 'appraisalintrialsubscription_Users', 'appraisal_Items','appraisal_AppraiserDetail-AppraiserNames','appraisal_AppraiserDetail-AppraiserTitles','appraisal_Items_Diamonds','appraisal_Items_ColoredStones','appraisal_Items_Pearls','appraisal_Items_MetalGroupPrices']
    #entity_list=['person']
    ...
    try:
        HISTORY_CHECK = Variable.get('appraisal_history_check')
    except:
        Variable.set('appraisal_history_check', 'True')
        HISTORY_CHECK = Variable.get('appraisal_history_check')
    ...
    for entity in entity_list:
        ...
        if '_' not in entity and '-' not in entity:
