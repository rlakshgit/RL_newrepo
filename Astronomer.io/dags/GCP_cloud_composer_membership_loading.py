...
from plugins.operators.jm_membership_staging import MembershipStagingOperator
...
with open(confFilePath, 'r') as configFile:
    ...
    membership_tables = confJSON.keys()

with DAG('zing_membership_loading_dag',
        schedule_interval= '0 11 * * *', #"@daily",#dt.timedelta(days=1), #
        catchup=True,
        max_active_runs=2,
    ...
    for table in membership_tables:
        if '_' in table:
            ...
            external_dag_id='zing_membership_landing_dag',
        ...
        stage_files = MembershipStagingOperator(task_id = 'stage_files_{source}_{table}'.format(source = source , table = table ),
                                                 source = source_abbr,
                                                 table = table,
                                                 config_file = confJSON,
                                                 google_cloud_storage_conn_id=base_gcp_connector,
        ...
        load_data_to_BQ = GoogleCloudStorageToBigQueryOperator(
                        ...
                        bucket=base_bucket,
                        source_objects=[
                        '{staging_file_location}{api_name}/{date}/staging_{source}_{api_name}*.csv'.format(staging_file_location=staging_file_location,
                                                                                                       source = source,
                                                                                        api_name=table,
                                                                                        date="{{ ds_nodash }}")],
        ...
        bq_load_audit_check = APIAuditOperator(
            ...
            source=source,
            source_abbr=source_abbr.lower(),
            metadata_filename='{location_base}{api_name}/{date}/l1_metadata_{source}_{api_name}.json'.format(
            ...
            audit_filename='{location_base}{api_name}/{date}/l1_audit_{source}_{api_name}.json'.format(
                ...
                date="{ds_nodash}"),
                #prefix=prefix),
            table='t_{source_abbr}_{table}'.format(source_abbr=source_abbr, table=table),
            check_landing_only=False
        )

        create_dataset >> task_sensor_core>> generate_schema >> stage_files >> load_data_to_BQ >> bq_load_audit_check>>end_task