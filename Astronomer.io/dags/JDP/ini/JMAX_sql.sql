WITH jmax_base AS (SELECT DISTINCT
       ...
       jmax_intermediate AS (SELECT DISTINCT
                    customer_number
                   , customer_name
                   , customer_address_1
                   , customer_address_2
                   ...
                   , customer_city
                   , customer_state
                   , SPLIT(customer_zip_code, '-')[OFFSET(0)]], ' '))) as JMAX_LocationKey
       ...
       current_jdp_jmax AS (SELECT DISTINCT(JMAX_LocationKey)
                   FROM `{project}.{dataset_jdp}.{table}`
                   WHERE date_created < DATE('{{{{ ds }}}}'))


 SELECT *, DATE('{{{{ ds }}}}') as date_created
 FROM jmax_intermediate
 WHERE JMAX_LocationKey not in (SELECT * FROM current_jdp_jmax)
