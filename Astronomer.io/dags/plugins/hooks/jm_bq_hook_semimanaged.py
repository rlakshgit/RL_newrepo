...
from pandas_gbq.gbq import (
    GbqConnector, _test_google_api_imports as gbq_test_google_api_imports,
)
...
class BigQueryHook(GoogleBaseHook):
    ...
    def __init__(self,
                 ...
                 api_resource_configs: Optional[Dict] = None, *args, **kwargs) -> None:
        ...
        super(BigQueryHook, self).__init__(
            gcp_conn_id=gcp_conn_id, delegate_to=delegate_to, *args, **kwargs)
    ...
    def get_pandas_df(
        ...
        try:
            GCP_CLOUD_COMPOSER = Variable.get('GCPCloudComposer')
        except:
            Variable.set('GCPCloudComposer', 'False')
            GCP_CLOUD_COMPOSER = Variable.get('GCPCloudComposer')

        if GCP_CLOUD_COMPOSER.upper() == 'FALSE':
            if dialect is None:
            ...
            # private_key = self.__get_field('key_path', None) or self.__get_field('keyfile_dict', None)
            try:
            ...
            try:
                print('Trying Pandas_GBQ no credentials Legacy Based')
                df = pd_gbq.read_gbq(sql,
                                     project_id=self._get_field('project'),
                                     verbose=True)
                # ,credentials=None)
            except:
                print('Trying Pandas_GBQ no credentials Standard Based')
                df = pd_gbq.read_gbq(sql,
                                     project_id=self._get_field('project'),
                                     verbose=True,
                                     dialect='standard')
    ...
    def gcp_bq_table_schema_update(self, project_id, dataset, table, new_col, datatype='STRING', null_base='NULLABLE'):
        '''
        ...
        '''
        gcp_hook = GoogleBaseHook(gcp_conn_id=self.gcp_conn_id)
        creds_data = gcp_hook._get_field('keyfile_dict')
        # self.connection = BaseHook.get_connection(gcp_conn_id)
        # self.creds_data = self.connection.extras["extra__google_cloud_platform__keyfile_dict"]
        info = json.loads(creds_data)
        credentials = service_account.Credentials.from_service_account_info(info)
        bigquery_client = bigquery.Client(credentials=credentials, project=project_id)
        dataset_id = dataset
        table_id = table
        logging.info('updating schema: ' + new_col)
        ...
        bigquery_client.update_table(table, ['schema'])  # API request
        return new_schema
            ...
            if schema_enable:
                ...
                if schema == 'NA':
                    ...
                    pd_gbq.to_gbq(df_in, destination_table=source_target,
                                  ...
                                  if_exists=control.lower(), credentials=credentials, verbose=True,
                ...
                pd_gbq.to_gbq(df_in, destination_table=source_target,
                              project_id=project.lower(),
                              if_exists=control.lower(), credentials=credentials,
