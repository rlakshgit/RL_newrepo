...
class GoogleAnalyticsHook(BaseHook):
    ...
    _key_folder = os.path.join(conf.get('core', 'dags_folder'), 'keys')
 ...
 #           self.client_secrets = self.connection.extra_dejson['client_secrets']
        self.client_secrets = json.loads(self.connection.extra_dejson['keyfile_dict'])
    ...
    def get_service_object(self, name):
        ...
        if self.connection.password:
         ...
         #                                                                  service.scopes)
            credentials = service_account.Credentials.from_service_account_info(self.client_secrets, scopes=service.scopes)

        elif hasattr(self, 'file_location'):
#            credentials = ServiceAccountCredentials.from_json_keyfile_name(self.file_location,
 #                                                                          service.scopes)
            credentials = service_account.Credentials.from_service_account_file(self.file_location, scopes=service.scopes)
        else:
