...
class CurrentVersionOperator(BaseOperator):
    """
    ...
    @apply_defaults
    def __init__(self,
                 sql: Optional[Union[str, Iterable]]=None,
                 source_task: Optional[str]=None,
                 sql_table: Optional[str] = None,
                 source: Optional[str] = None,
                 mssql_conn_id: Optional[str] = None,
                 build_out_source_table: Optional[str]=None,
                 ...
                 change_tracking_check: Optional[bool] = False,
                 *args,
                 **kwargs) -> None:
        super().__init__(*args, **kwargs)
        ...
        self.source = source
        self.build_out_source_table = build_out_source_table
    ...
    def execute(self, context):
        #get primary key from hook instead of xcom(all in task)
        primary_key = self._get_primary_key()
        #primary_key = context['task_instance'].xcom_pull(task_ids=self.source_task)
        self.log.info('primary key: %s', primary_key)

        # build_out_source_table = '''{project}.{dataset}.{table}'''.format(project=self.source_project,dataset=self.source_dataset,table=self.table)
        #Get list of primary keys from mssql and use it in the below queries


        if primary_key != 'None' and self.change_tracking_check:
            self.sql = '''select j.* EXCEPT (SYS_CHANGE_OPERATION, CT_ID),Date('{bq_load_date}') as bq_load_date from (select a.*,DATE(_PARTITIONTIME) as pd from `{build_out_source_table}` a) j 
                                        INNER JOIN (SELECT {ID}, MAX(DATE(_PARTITIONTIME)) as pd_datetime FROM `{build_out_source_table}` b  where DATE(_PARTITIONTIME) <= Date('{bq_load_date}')
                                        GROUP BY b.{ID} order by pd_datetime ASC) c ON c.{ID} = j.{ID} and c.pd_datetime = j.pd 
                                        where j.{ID} not in (select CT_ID FROM `{build_out_source_table}` where SYS_CHANGE_OPERATION = 'D' )'''.format(
                build_out_source_table=self.build_out_source_table, ID=primary_key,bq_load_date=context['ds'])
        else:
            self.sql = ''' select *EXCEPT (SYS_CHANGE_OPERATION, CT_ID),Date('{bq_load_date}') as bq_load_date  from `{build_out_source_table}` 
                             where DATE(_PARTITIONTIME) = (select MAX(DATE(_PARTITIONTIME)) 
                             from `{build_out_source_table}`)  '''.format(
                build_out_source_table=self.build_out_source_table,bq_load_date=context['ds'])
                           ...
                           INNER JOIN
                               INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS KU
                                     ON TC.CONSTRAINT_TYPE = 'PRIMARY KEY' AND
                                        ...
                                        KU.table_name = '{table}'
                           ORDER BY KU.TABLE_NAME, KU.ORDINAL_POSITION;'''.format(table=self.sql_table)

        result = self._query_mssql(sql=sql, database=self.source)
