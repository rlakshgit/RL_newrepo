...
class ExperianAuditOperator(BaseOperator):
    """
    ...
    **Example**:
    ...
    def __init__(self,
        ...
        super(ExperianAuditOperator, self).__init__(*args, **kwargs)
        self.sql = sql
        self.bucket = bucket
    ...
    def execute(self, context):
        ...
        print('*' * 20)
        print(self.landing_file_location)
        ...
        try:
            value = metadata_json['landing_row_count']
        except:
            metadata_json['landing_row_count'] = 0
        print(metadata_json)

        audit_result = 'PASSED'
        audit_reason = ''
        audit_dt = dt.datetime.now().strftime("%Y%m%d%H%M%S")
        if metadata_json['landing_row_count'] == 0 and metadata_json['landing_file_count'] == 0:
        # if metadata_json['landing_file_count'] == 0:
            json_data = {'file_count': metadata_json['file_count'],
                         'landing_row_count': metadata_json['landing_row_count'],
                         'landing_file_count': metadata_json['landing_file_count'],
                         'bq_row_count': 'NOT CHECKED',
            ...
            if table_audit_results.loc[0, 'count'] != metadata_json['landing_row_count']:
                audit_result = 'FAILED'
            ...
            json_data = {'file_count': metadata_json['file_count'],
                         'landing_row_count': metadata_json['landing_row_count'],
                         'landing_file_count': metadata_json['landing_file_count'],
                         'bq_row_count' : table_audit_results.loc[0, 'count'],
                         'dag_execution_date': context['ds'],
                         'audit_execution_date': audit_dt,
                         'audit_result': audit_result,
                         'audit_reason': audit_reason
    ...
    def _upload_audit_results(self,json_data):
        ...
        gcs_hook.upload(self.bucket,
                        self.metadata_filename + '/experian_audit.json',
    ...
    def _landing_check(self,context):
        ...
        gcs_hook = GCSHook(self.google_cloud_bq_conn_id)
        file_list = gcs_hook.list(self.bucket, prefix=self.landing_file_location)
        ...
        audit_dt = dt.datetime.now().strftime("%Y%m%d%H%M%S")
        json_data = {'file_count': metadata_json['file_count'],
                     'landing_file_count': len(file_list),
                     'dag_execution_date': context['ds'],
                     'audit_execution_date': audit_dt,
                     'audit_result': audit_result,
                     'audit_reason': audit_reason
