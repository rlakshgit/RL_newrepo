...
class ExperianSFTPtoGCS(BaseOperator):
    """
    ...
    def execute(self, context):
        ...
        history_check = True
        if history_check:
            file_list = gcs_hook.list(self.destination_bucket, maxResults=1000, prefix=self.destination_path)
            if len(file_list) == 15:
                print('Found file count of ....', len(file_list))
                print('Not attempting download again')
                return
        ...
        # This checks to see if the directory has been updated, but doesn't exclude any files
        ...
        if context['ds_nodash'] != modified_date:
            print('No new files')
            self._upload_audit_results(0, context)
            return
        ...
        for file in files:
            ...
            # Get the modified date of the current file.  If it doesn't match the dag execution date, skip.
            modified_date_full = sftp_hook.get_mod_time(source_path)
            modified_date = modified_date_full[:8]
            print(
                'File {this_file} modified date is {this_date}'.format(this_file=source_path, this_date=modified_date))
            if context['ds_nodash'] != modified_date:
                continue

            file_counter += 1
            try:
                bypass = Variable.get('BYPASS').upper()
            except:
                bypass = 'FALSE'
        ...
        self._upload_audit_results(file_counter, context)

    def _upload_audit_results(self, file_count, context):
        gcs_hook = GCSHook(self.gcp_conn_id)

        audit_dt = dt.datetime.now().strftime("%Y%m%d%H%M%S")
        json_data = {'file_count': file_count,
        ...
        if self.audit_location != 'NA':
            gcs_hook.upload(self.destination_bucket,
