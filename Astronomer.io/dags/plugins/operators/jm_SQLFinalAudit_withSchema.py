...
class SQLFinalAuditOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        for table in self.table_list:
            ...
            schema = table_split[0].replace(' ', '')
            table = table_split[1].replace('$', '')
            if schema != 'dbo':
               # print('self.destination_dataset')
                #print(self.destination_dataset)
                destination_dataset = self.destination_dataset
                destination_dataset = destination_dataset.replace('_current', '_' + schema.lower() +'_current')
                #self.destination_dataset = destination_dataset
                #print(self.destination_dataset)
            else:
                destination_dataset = self.destination_dataset
            ...
            if table.startswith('cctl_') or table.startswith('pctl_') or table.startswith('bctl_'):
            ...
            final_df.loc[i,'sql_count'] = str(df.loc[df.TableName == table].loc[df.schema == schema,'sql_count'].iloc[0])
            if str(result_df.iloc[0,0]) == str(df.loc[df.TableName == table].loc[df.schema == schema,'sql_count'].iloc[0]):
        ...
        if self.hook.table_exists(project_id=self.project, dataset_id=destination_dataset,
                                         table_id=self.destination_table):
            logging.info('Destination table {} already exists!'.format(self.destination_table))
        else:
            self.hook.create_empty_table(project_id=self.project, dataset_id=destination_dataset,
                                                table_id=self.destination_table,
