...
class GoogleCloudStorageToBigQueryOperator(BaseOperator):
    ...
    def __init__(self,
        ...
        self.gcs_to_bq = gcs_to_bq

    def execute(self, context):
        print(self.bucket)
        print(self.source_objects)
        print(self.schema_object)
        print(self.destination_project_dataset_table)
        
        [load_proj, load_dataset, load_table] = self.destination_project_dataset_table.split(".")
        print('rlaksh 0901 : project_id ',self.project_id)
        if self.gcs_to_bq.upper() == 'FALSE':
        ...
        print('rlaksh 0901 : after bq_hook ')
        if self.schema_object != None:
            ...
            try:
                ...
                print('loading schema file from:')
                print(self.bucket, self.schema_object)
        ...
        print(source_uris)
        print('rlaksh 0901 : before conn ')
        conn = bq_hook.get_conn()
        #conn1 = bq_hook.get_client()
        print('rlaksh 0901 : after conn ', self.project_id)
        cursor = conn.cursor()
        print('rlaksh 0901 : after cursor - time_partitioning ',self.time_partitioning)

        job_configuration = {
                    "load": {
                        "destinationTable": {
                            "project_id": load_proj,
                            "datasetId": load_dataset,
                            "tableId": load_table,
                        },
