...
class GoogleCloudStorageToBigQueryOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        eas_files_availability = 'eas_files_availability_{schema_item}'.format(schema_item=self.schema_item)
        print(" int value eas_files_availability",eas_files_availability, int(Variable.get(eas_files_availability)))
        if (int(Variable.get(eas_files_availability)) == 0):
            print("No data to process for ",eas_files_availability)
            return

        print("Proceeding with load for ",eas_files_availability)
        ...
        cursor = conn.cursor()
        print("source_urls", source_uris, "  type of it", type(source_uris))
