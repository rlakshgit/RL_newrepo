...
class Mongodbtogcs(BaseOperator):
    ...
    def execute(self, context):
        ...
        response = mycollection.find({'$and': [{"updatedOn.instant.DateTime": {'$gte': start_date}},{"updatedOn.instant.DateTime": {'$lt': end_date}}]})
        ...
        for array in arrays_list:
            ...
            table_name = self.entity + '_' + array
            parent_pk = self.confJSON[table_name]["parent_pk"] # get primary key column name from config file
            date_fields_list = self.confJSON[table_name]["date_fields"] # get list of date fields to be processed from config file 
            array_df, source_count = self._flatten_array(list_data, array, parent_pk) 
            array_name = '_' + array
            array_df2 = self._split_date_field(array_df, date_fields_list)
            logging.info(f'UPLOADING {array} FILE TO GCS')
            write_return = self._upload_normalized_data_gcs(array_df2, context, array_name=array_name)
        ...
        parent_date_fields_list = self.confJSON[self.entity]["date_fields"]
        for dict in list_data:
            ...
            # split date fields 
            # logging.info(f"SPLITTING DATE FIELDS FOR: {dict['_id']} : {dict['createdOn']}")
            df_norm = self._split_date_field(df_norm, parent_date_fields_list)



        logging.info("UPLOADING FLAT FILE TO GCS")
    ...
    def _split_date_field(self, df, date_fields_list):
        '''
        This function split a DataTime field into 2 seperate columns (data & time)
        and keep the original field as is
        and returns the new DF
        '''

        for col in df:
            if col in date_fields_list:
                try:
                    new_date_field = col+'_date'
                    new_time_field = col+'_time'
                    df[new_date_field] = pd.to_datetime(df[col].astype(str), errors='coerce')
                    df[new_date_field] = df[new_date_field].dt.strftime('%Y-%m-%d')
                    df[new_time_field] = pd.to_datetime(df[col].astype(str), errors='coerce')
                    df[new_time_field] = df[new_time_field].dt.strftime('%H:%M:%S.%f')
                except Exception as e:
                    logging.warning(f'UNABLE TO SPLIT THE DATE FIELD: {col}')
                    print(e)
                    new_date_field = col+'_date'
                    new_time_field = col+'_time'
                    df[new_date_field] = ''
