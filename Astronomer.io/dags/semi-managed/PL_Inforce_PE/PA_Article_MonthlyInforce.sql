...
--  `semi-managed-reporting.core_insurance_pjpa.nonpromoted_PA_Article_MonthlyInforcePremium`
--  (AsOfDate DATE
--  ,ProductLine STRING
--  ,LOBCode STRING
--  ,PolicyNumber STRING
...
WITH transactions AS (
    SELECT
        ...
        ,COALESCE(Article.ItemNumber, 1) AS ItemNumber
    ...
    FULL OUTER JOIN	`{project}.{dataset_ref_pc_current}.pcx_personalarticle_jm` AS article
            ...
            AND pperiod.ID = article.BranchID
            AND article.IsItemInactive = false
