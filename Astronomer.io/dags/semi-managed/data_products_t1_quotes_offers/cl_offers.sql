...
SELECT
	...
	,CASE 
		WHEN vRS.RiskType = 'LOB' THEN vRS.PrimaryRatingLocationNumber 
		ELSE vRS.LocationNumber
		END																		AS LOCATION_NUMBER
	,CASE 
		WHEN vRS.RiskType = 'LOB' THEN vRS.PrimaryRatingBuildingNumber 
		WHEN vRS.RiskType = 'BLD' THEN vRS.BuildingNumber 
		ELSE vRS.LocationBuildingNumber
		END																		AS BUILDING_NUMBER
	,CASE 
		WHEN vRS.RiskType = 'LOB' THEN D_primary_geo.GeographyStateCode 
	...
	,CASE 
		...
		END																		AS HAS_UMB
	,CASE 
		WHEN vRS.LocationSegmentationCode LIKE '%Rol%' THEN 1 ELSE 0 
		END																		AS ROLEX_LOCATION
	,CASE 
		WHEN vRS.LocationSegmentationCode LIKE '%Pan%' THEN 1 ELSE 0
		END																		AS PANDORA_LOCATION
	,vRS.LocationTypeDesc														AS LOCATION_TYPE
	,vRS.StockPremiumBaseAOP													AS INVENTORY_PREMIUM_BASE
	,vRS.LocationInlandMarineHazardCode											AS HAZARD_CODE
	,vRS.LocationBuildingConstructionCode										AS CONST_CODE
	,vRS.LocationInlandMarineExperienceTierFactor								AS EXPERIENCE_TIER_FACTOR
	,vRS.BuildingYearOfConstruction												AS YR_CONSTRUCTION
	,vRS.StockIsPawnPropertyCoverageIncluded									AS JB_JS_PAWN_COVERED_INDICATOR
	...
	,DENSE_RANK() OVER(PARTITION BY D_policy.Policykey
						   ...
						   ORDER BY	vRS.RiskSegmentKey DESC
									,COALESCE(D_cov.CoverageKey,999999999) DESC
						)														AS COV_RANK
	,vRS.StockInventoryLooseDiamonds											AS StockInventoryLooseDiamonds				
	,vRS.StockInventoryLowValue													AS StockInventoryLowValue
	,vRS.StockInventoryHighValue												AS StockInventoryHighValue
	,vRS.StockInventoryLowValueWatches											AS StockInventoryLowValueWatches
	,vRS.StockInventoryHighValueWatches											AS StockInventoryHighValueWatches
	,vRS.StockInventoryScrapPreciousMetals										AS StockInventoryScrapPreciousMetals
	,vRS.StockInventoryNonJewelryPreciousMetals									AS StockInventoryNonJewelryPreciousMetals
	,vRS.StockInventoryOther													AS StockInventoryOther
	,vRS.StockHasLuxuryBrandWatches												AS StockHasLuxuryBrandWatches
	,vRS.StockHasWatchBlancpain													AS StockHasWatchBlancpain
	,vRS.StockHasWatchBreitling													AS StockHasWatchBreitling
	,vRS.StockHasWatchCartier													AS StockHasWatchCartier
	,vRS.StockHasWatchOmega														AS StockHasWatchOmega
	,vRS.StockHasWatchPatekPhilippe												AS StockHasWatchPatekPhilippe
	,vRS.StockHasWatchRolex														AS StockHasWatchRolex
	,vRS.StockHasWatchOther														AS StockHasWatchOther
	,vRS.IsStockDealerAnOfficialRolexDealer										AS IsStockDealerAnOfficialRolexDealer
	,CASE
		WHEN vRS.BuildingBOPInsuredsBusinessClassCode = '59982'
		THEN vRS.BuildingBOPInsuredsBusinessClassCode ELSE 0
		END																		AS PAWN_CLASS
...
FROM bi_dds.FactQuote AS F_quote
	...
	INNER JOIN bi_dds.DimLineOfBusiness AS D_LOB
		ON D_LOB.LineOfBusinessKey = vRS.LineOfBusinessKey

	INNER JOIN bi_dds.DimCoverage AS D_cov
		ON D_cov.RiskSegmentKey = vRS.RiskSegmentKey
	...
	INNER JOIN bi_dds.DimGeography AS D_loc_geo
		ON D_loc_geo.GeographyKey = vRS.LocationGeographyKey

	INNER JOIN bi_dds.DimGeography AS D_primary_geo
		ON D_primary_geo.GeographyKey = vRS.PrimaryRatingGeographyKey
...
GROUP BY
	...
	,vRS.RiskType
	,vRS.PrimaryRatingLocationNumber
	,vRS.LocationNumber
	,vRS.PrimaryRatingBuildingNumber
	,vRS.BuildingNumber
	,vRS.LocationBuildingNumber
	,D_primary_geo.GeographyStateCode
	,D_loc_geo.GeographyStateCode
	,D_LOB.LOBCode
	,vRS.LocationSegmentationCode
	,vRS.LocationTypeDesc
	,vRS.StockPremiumBaseAOP
	,vRS.LocationInlandMarineHazardCode
	,vRS.LocationBuildingConstructionCode
	,vRS.LocationInlandMarineExperienceTierFactor
	,vRS.BuildingYearOfConstruction
	,vRS.StockIsPawnPropertyCoverageIncluded
	,D_cov_type.ConformedCoverageTypeCode
	,D_cov.PerOccurenceLimit
	,D_cov.PerOccurenceDeductible
	,vRS.RiskSegmentKey
	,D_cov.CoverageKey
	,vRS.StockInventoryLooseDiamonds
	,vRS.StockInventoryLowValue
	,vRS.StockInventoryHighValue
	,vRS.StockInventoryLowValueWatches
	,vRS.StockInventoryHighValueWatches
	,vRS.StockInventoryScrapPreciousMetals
	,vRS.StockInventoryNonJewelryPreciousMetals
	,vRS.StockInventoryOther
	,vRS.StockHasLuxuryBrandWatches
	,vRS.StockHasWatchBlancpain
	,vRS.StockHasWatchBreitling
	,vRS.StockHasWatchCartier
	,vRS.StockHasWatchOmega
	,vRS.StockHasWatchPatekPhilippe
	,vRS.StockHasWatchRolex
	,vRS.StockHasWatchOther
	,vRS.IsStockDealerAnOfficialRolexDealer
	,vRS.BuildingBOPInsuredsBusinessClassCode
)
...
SELECT
	...
	,CASE
		...
		THEN JB_JS_Out_of_Safe_Limit / STOCK_LIMIT
										ELSE ''
