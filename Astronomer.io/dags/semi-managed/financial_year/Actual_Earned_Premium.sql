...
DECLARE @BegDate AS DATE = '{startDate}'
DECLARE @EndDate AS DATE = '{endDate}'
...
WHERE	1=1
		--AND LEFT(fMPE.DateKey,4) >= YEAR(SYSDATETIME())-4
		AND CAST(CAST(fMPE.DateKey AS VARCHAR(10)) AS date) BETWEEN @BegDate AND @EndDate
