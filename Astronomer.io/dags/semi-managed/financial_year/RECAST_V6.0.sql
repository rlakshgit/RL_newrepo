...
(
	SELECT
		...
		,CASE	WHEN cNB.SP_Source = 'GEICO Insurance Agency Inc' THEN 'SP - GEICO'
				...
				WHEN cNB.SP_Source = 'Matic Insurance Services' THEN 'SP - Matic'
				WHEN cNB.SP_Source <> '' THEN 'SP - Other'
				WHEN cJCD.JC_ProgramDesc = 'LINK' AND cNB.AgencyMasterCode = 'LUX' THEN 'JP - Luxsurance'
				WHEN cNB.RecastSourceOfBusiness = 'LINK' AND cNB.AgencyMasterCode = 'LUX' THEN 'JP - Luxsurance'
				...
				WHEN cNB.RecastSourceOfBusiness in ( 'Point of Sale', 'JMCC') THEN 'Jeweler Programs'
				...
				WHEN (cNB.RecastSourceOfBusiness = 'Express') AND cNB.JewelerName = 'PL Customer Brochure' THEN 'eCommerce'
...
SELECT
	...
	,CASE	WHEN cG.Recast = 'SP - GEICO' AND cG.GeicoSubCodeDesc = 'Call Center - Honolulu' THEN 'SP - Other' ELSE cG.Recast END AS Recast
	,CASE	WHEN cG.Recast = 'SP - GEICO' THEN cG.GeicoSubCodeDesc
			WHEN cG.Recast like 'SP%' THEN cG.SP_Source
			WHEN cG.Recast = 'JP - Luxsurance' THEN 'Luxsurance'
			WHEN cG.Recast = 'Jeweler Programs' THEN 
					CASE	WHEN cG.JC_ProgramDesc IN ('Jeweler''s Cut','JC CAP') THEN 'JC Paper & CAP'
							...
							WHEN cG.RecastSourceOfBusiness in ( 'Point of Sale', 'JMCC') THEN 'Customer Connect'
			...
			WHEN cG.Recast = 'Express' THEN
					...
					END
			WHEN cG.Recast = 'eCommerce' AND cG.JewelerName = 'PL Customer Brochure' THEN 'Brochure'
			ELSE cG.Recast
		END AS SubRecast
...
GROUP BY
	...
	,CASE	WHEN cG.Recast = 'SP - GEICO' AND cG.GeicoSubCodeDesc = 'Call Center - Honolulu' THEN 'SP - Other' ELSE cG.Recast END 
	,CASE	WHEN cG.Recast = 'SP - GEICO' THEN cG.GeicoSubCodeDesc
			WHEN cG.Recast like 'SP%' THEN cG.SP_Source
			WHEN cG.Recast = 'JP - Luxsurance' THEN 'Luxsurance'
			WHEN cG.Recast = 'Jeweler Programs' THEN 
